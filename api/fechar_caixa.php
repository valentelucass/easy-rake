...
// api/fechar_caixa.php (Versão que gera relatório final completo)
...
$observacoes = $data['observacoes'] ?? ''; // Nova entrada: Observações do fechamento

// Inicia uma transação para garantir a consistência
$conn->beginTransaction();

try {
    // 1. Busca o resumo da sessão (já existe)
    $summaryStmt = $conn->prepare("SELECT * FROM cashier_session_summary WHERE id = ?");
    $summaryStmt->execute([$sessionId]);
    $summary = $summaryStmt->fetch(PDO::FETCH_ASSOC);

    if (!$summary) {
        throw new Exception('Nenhum caixa aberto encontrado ou falha ao buscar resumo.');
    }

    // 2. Coleta de dados DETALHADOS para o relatório
    $reportDetails = [];
    ...
    $reportDetails['resumo_financeiro'] = $summary; // Usa o resumo da view como base
    $reportDetails['resumo_financeiro']['final_amount'] = $finalAmount; // Adiciona o valor real conferido

    // Calcula a diferença de fichas: Fichas Vendidas - (Devoluções + Rake + Caixinhas)
    // Nota: 'Caixinhas' para este cálculo precisa vir da soma do cashback, que buscaremos abaixo
    $totalCashbackCaixinhas = 0; // Será preenchido na seção de caixinhas

    // 2.1. Transações Detalhadas (vendas, devoluções, rake, despesas, pagamentos de débito)
    ...
    $stmt_players_session = $conn->prepare(
        "SELECT p.id, p.name, p.cpf, p.telefone FROM players p 
         ...
         GROUP BY p.id, p.name, p.cpf, p.telefone" // Inclui CPF e Telefone aqui
    );
    ...
    foreach ($players as $player) {
        ...
        // Encontra a data da ÚLTIMA quitação na sessão para calcular saldo atual
    ...
    if ($_SESSION['perfil'] === 'gestor') {
        ...
        foreach ($caixinhas as $caixinha) {
            ...
            $totalCashbackCaixinhas += $cashback_retido; // Soma para o cálculo da diferença de fichas
    ...
    // Atualiza o total de cashback para o resumo financeiro
    $reportDetails['resumo_financeiro']['total_cashback_caixinhas'] = $totalCashbackCaixinhas;

    // Calcula a diferença de fichas final (agora com o valor das caixinhas)
    ...
    // 2.6. Observações e Assinatura
    $reportDetails['observacoes'] = $observacoes; // Captura observações enviadas do frontend
    $reportDetails['assinatura'] = '________________________________'; // Placeholder para assinatura física

    // 3. Converte o relatório para JSON e salva no banco de dados
    ...
    // 4. Atualiza a sessão de caixa para 'closed' (lógica existente)
    ...
    if (!$success || $stmt->rowCount() == 0) {
        throw new Exception('Falha ao fechar o caixa no banco de dados.');
    }

    $conn->commit();
    echo json_encode(['success' => true, 'report' => $reportDetails]); // Retorna o relatório final completo

} catch (Exception $e) {
    $conn->rollBack();
    error_log('Erro ao fechar caixa e gerar relatório: ' . $e->getMessage());
